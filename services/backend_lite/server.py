...
def main():
    log_dir = Path(config.PROJECT_ROOT) / 'services' / 'backend_lite' / 'output' / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / 'lite_backend.log'
    ...
    logging.info(f'Lite backend running at http://{host}:{port}')
    logging.info(f'Serving frontend from: {config.WEB_ROOT}')
    logging.info(f'Using sqlite database: {config.DATA_FILE.with_name("smartwardrobe_lite.db")}')
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        logging.info('Lite backend stopped.')
    finally:
        server.server_close()
