...
4. 如果用户输入包含 Context/Source/Summary，你必须基于该真实材料写作，必要时使用联网搜索补全关键信息（时间、地点、品牌/设计师/系列名、关键造型细节），禁止脱离事实泛泛而谈。
5. 提供适合本文的图片检索策略，而不是死板的风格标签。
{visual_strategy}
6. 输出必须是纯粹的、符合规范的 JSON 格式。不能包含任何 Markdown 符号或多余的文字。
</rules>
