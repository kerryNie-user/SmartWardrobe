...
class AutoLocationService {
    ...
    updateGlobalLocationState(location) {
        ...
        const locationText = this.translateAutoSelect(location);
    ...
    updateStatusUI(status, text = '', regionKey = '') {
        ...
        if (status === 'loading') {
            ...
            if (!statusEl.innerHTML.includes('location-highlight')) {
                const detectingText = window.i18n?.get('detecting_location') || 'Detecting...';
                statusEl.innerHTML = `<div class="spinner" style="width:14px;height:14px;border-width:2px;"></div> <span style="font-size:12px;color:var(--text-sub);">${detectingText}</span>`;
            }
            ...
            statusEl.innerHTML = `<span class="location-error" style="color:var(--danger-color);font-size:12px;">${errorText}</span>`;
        }
    }
    ...
    translateAutoSelect(locationData) {
        ...
        const mapTerm = (term, type) => {
            ...
            // Special cases
            if (term === 'IP City' || term === 'Unknown City') return i18n.get('location_ip_city') || term;
            if (term === 'Unknown Province' || term === 'Unknown Region') return i18n.get('location_unknown_province') || term;
