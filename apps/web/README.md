...
- 同源服务 `apps/web`
- 暴露 `/api/*`
- 使用 `services/backend/data/db.json` 作为本地联调数据文件
...
```bash
node apps/web/tests/home.shell.test.js
node apps/web/tests/me.shell.test.js
node apps/web/tests/schedule.shell.test.js
node apps/web/tests/api-connect.shell.test.js
```

## 当前数据边界

- 页面层尽量通过 `js/lib/*Store.js` 取数与写入
- `js/lib/backendClient.js` 负责与轻量后端通信
- 当前策略是：
  - 本地缓存继续保留
  - 页面初始化时异步 hydrate
