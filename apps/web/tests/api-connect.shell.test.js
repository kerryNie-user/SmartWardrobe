...
async function main() {
  await runTest('New store 应从 backend hydrate 资料与设置', async () => {
  ...
  await runTest('New store 应从 backend hydrate 收藏与衣橱', async () => {
    const dom = new JSDOM('<!doctype html><html><body></body></html>', { url: 'http://localhost/me.html' });
    const { fetch, requests } = createFetchStub();
  ...
  await runTest('New Schedule store 应从 backend hydrate 并写回日程', async () => {
