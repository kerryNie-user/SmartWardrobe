...
runTest('ownerId 应只来自 ct_auth_session 而不是 currentUser 回退', async () => {
  ...
  const backendModulePath = `${pathToFileURL(path.join(__dirname, '..', 'js', 'lib', 'backendClient.js')).href}?owner-source=1`;
  const { getCurrentScopedUserId } = await import(scopedModulePath);
  const { getBackendUserId } = await import(backendModulePath);

  assert.strictEqual(getCurrentScopedUserId(), 'guest');
  assert.strictEqual(getBackendUserId(), null);
});
