...
export function toggleFavorite(type, item) {
    ...
    if (existingIndex >= 0) {
        ...
        void requestBackend(`/api/favorites/${normalizedType}/${item.id}`, {
            method: 'DELETE'
        }).then((response) => {
            if (!response.ok) {
    ...
    favoritesSyncController.markSyncing()
    void requestBackend('/api/favorites', {
        method: 'POST',
        payload: {
            type: normalizedType,
            item: normalizeItem(normalizedType, item)
...
export function removeFavorite(type, id) {
    ...
    favoritesSyncController.markSyncing()
    void requestBackend(`/api/favorites/${normalizedType}/${id}`, {
        method: 'DELETE'
    }).then((response) => {
        if (!response.ok) {
            writeFavorites(previousFavorites, scope)
...
export async function hydrateFavorites() {
    ...
    const remote = await requestBackend('/api/favorites')
...
export function retryFavoritesSync() {
    ...
    if (mutation.kind === 'add') {
        ...
        return requestBackend('/api/favorites', {
            method: 'POST',
            payload: {
                type: mutation.type,
                item: mutation.item
    ...
    return requestBackend(`/api/favorites/${mutation.type}/${mutation.id}`, {
        method: 'DELETE'
    }).then((response) => {
        if (!response.ok) {
            writeFavorites(previousFavorites, scope)
